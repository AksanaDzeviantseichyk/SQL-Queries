...
SELECT 
    ...
    pr.max_sum_rate -(pos.rate + ISNULL(SUM(eq.price),0)/12) AS difference
FROM employee emp
...
GROUP BY emp.first_name, emp.last_name, pr.name, pr.max_sum_rate, pos.rate
HAVING (pos.rate + ISNULL(SUM(eq.price),0)/12) > pr.max_sum_rate